...
'''Creates a regularly spaced square package of circles. The square array id disturbed by first crisscrossing them.
...
x = np.linspace(0+(eq_dis+d/2), shape[1]-(eq_dis+d/2), num_x, dtype=np.float)
y = np.linspace(0+(eq_dis+d/2), shape[0]-(eq_dis+d/2), num_y, dtype=np.float)
    ...
    for i in range(len(coords)):
        ...
        while True:
            ...
            if (d/2 + dist_border) <= coords[i][0] + rho*np.cos(theta) <=  shape[1]-(dist_border + d/2) and \
               (d/2 + dist_border) <= coords[i][1] + rho*np.cos(theta) <=  shape[1]-(dist_border + d/2):
